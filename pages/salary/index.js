...
import techTable from '../../template/tech-table';
...
Page({
  ...
  getNewAverageSalary: function() {
    salaryService.getAverageSalary().then(({
      monthSalary,
      latestSalary
    }) => {
  ...
  getChartsSalaryInfo: function() {
    salaryService.getChartsSalaryInfo().then(({
  ...
  viewCompanyDetail: function(event) {
    techTable.viewCompanyDetail(event);
  }
